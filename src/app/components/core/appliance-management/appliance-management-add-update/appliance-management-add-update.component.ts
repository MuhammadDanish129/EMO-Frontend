...
export class ApplianceManagementAddUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.isEditMode && this.data.value) {
      this.model = {
        ...
        isShiftable: this.data.value.isShiftable ?? false,
        isCritical: this.data.value.isCritical ?? false,
        priorityLevel: this.data.value.priorityLevel || 'Normal',
        normalOperatingHours: this.data.value.normalOperatingHours || '',
        canAutoControl: this.data.value.canAutoControl ?? false,
        allowedShiftStartTime: this.data.value.allowedShiftStartTime || '',
        allowedShiftEndTime: this.data.value.allowedShiftEndTime || '',
        allowOptimizationSuggestions: this.data.value.allowOptimizationSuggestions ?? true,
        minimumOnDurationMinutes: this.toNumber(this.data.value.minimumOnDurationMinutes),
        minimumOffDurationMinutes: this.toNumber(this.data.value.minimumOffDurationMinutes),
  ...
  save() {
    ...
    const payload: ApplianceRequestDTO = {
      ...
      isShiftable: this.model.isShiftable ?? false,
      isCritical: this.model.isCritical ?? false,
      priorityLevel: this.model.priorityLevel || 'Normal',
      normalOperatingHours: (this.model.normalOperatingHours || '').trim(),
      canAutoControl: this.model.canAutoControl ?? false,
      allowedShiftStartTime: this.model.isShiftable ? (this.model.allowedShiftStartTime || '').trim() : '',
      allowedShiftEndTime: this.model.isShiftable ? (this.model.allowedShiftEndTime || '').trim() : '',
      allowOptimizationSuggestions: this.model.allowOptimizationSuggestions ?? true,
      minimumOnDurationMinutes: this.toNumber(this.model.minimumOnDurationMinutes),
      minimumOffDurationMinutes: this.toNumber(this.model.minimumOffDurationMinutes),
  ...
  private createDefaultModel(): ApplianceRequestDTO {
    return {
      ...
      canAutoControl: false,
      allowedShiftStartTime: '',
      allowedShiftEndTime: '',
      allowOptimizationSuggestions: true,
      minimumOnDurationMinutes: 0,
      minimumOffDurationMinutes: 0,
      isDefault: false,
      isCustom: true,
      isActive: true,
      fkUtility: ''
